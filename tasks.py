...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-2fa-social-auth runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-2fa-social-auth.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ social_2fa")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-2fa-social-auth tests")
...
def release(c, bumpsize=''):
    ...
    import dj_2fa_social_auth
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_2fa_social_auth.__version__))
    c.run("git push --tags")
